    ...
    : IRequestHandler<GetAllHeroesRequest, PaginatedList<GetHeroResponse>>
{
    ...
    public async Task<PaginatedList<GetHeroResponse>> Handle(
        GetAllHeroesRequest request,
    ...
    )
    {
        var heroes = _context.Heroes
            ...
            .WhereIf(
                !string.IsNullOrEmpty(request.Individuality),
                x => EF.Functions.Like(x.Individuality!, $"%{request.Individuality}%")
            );
        return await heroes
            .ProjectToType<GetHeroResponse>()
            .OrderBy(x => x.Name)
