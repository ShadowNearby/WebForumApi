...
{
    private static readonly string _dataPath = Directory.GetParent(Environment.CurrentDirectory)!.Parent!.FullName;
    public async Task<Result<FileDto>> Handle(FileUploadRequest request, CancellationToken cancellationToken)
    {
        IFormFile file = request.File;
        string filename = $"{Guid.NewGuid().ToString()}{file.FileName}";
        string filepath = $"{_dataPath}/wwwroot/images/{filename}";
